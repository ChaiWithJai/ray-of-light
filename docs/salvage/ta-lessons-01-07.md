...
| 2.2 | Either/or question = two -ஆ questions (ஸ்ட்ராங்கா வேணுமா, லைட்டா வேணுமா?) line + grammar note | Main teaches single -ஆ questions only | PORT | main l2, beside the -ஆ teaching on `ta.irukka-q` (or into main l1's coffee scene as a customization beat) | One line max; strong/light loanwords are authentic spoken register. |
...
| 3.3 | ரூபா as spoken form of ரூபாய் (accepted variants) | Main consistently writes ரூபாய் | PORT | main l3/l5, translit note + accepted variants where ரூபாய் appears | Variant-level only; canonical stays ரூபாய் pending native review. |
